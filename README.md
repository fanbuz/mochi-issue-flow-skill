...
状态查询使用 `scripts/flow_status.py`；证据归档使用 `scripts/archive_flow_evidence.py`；上下文预算使用 `scripts/check_context_budget.py`。平台 adapter 必须在结果进入模型前完成 comment 定向过滤，并只返回一份规范化载荷。
